...
export class RepoVerificationService {
  ...
  async verifyRepo(
    ...
    if (credentialId) {
      try {
        const token =
          await this.credentialService.getDecryptedValue(credentialId);
        ...
        return {
          accessible: false,
          message: `Failed to decrypt credential: ${error.message}`,
        };
      }
    }
  ...
  async verifyPromotionRepo(projectId: string): Promise<VerificationResult> {
    const repo = await this.prisma.promotionRepo.findUnique({
      where: { projectId },
    });
    if (!repo) {
  ...
  async verifySourceRepo(repoId: string): Promise<VerificationResult> {
    const repo = await this.prisma.sourceRepo.findUniqueOrThrow({
      where: { id: repoId },
    });
