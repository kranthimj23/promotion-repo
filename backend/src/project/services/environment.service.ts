...
export class EnvironmentService {
  ...
  private readonly defaultTemplate = [
    ...
    },
    {
      ...
      promotionOrder: 5,
      valuesFolder: 'prod-values',
      isProduction: true,
    },
  ];

  async applyTemplate(projectId: string) {
    const existing = await this.prisma.environment.count({
      where: { projectId },
    });
    if (existing > 0) {
      throw new ConflictException(
        'Project already has environments. Remove them first or add individually.',
      );
    }
  ...
  async addEnvironment(projectId: string, dto: CreateEnvironmentDto) {
    const existing = await this.prisma.environment.findFirst({
      where: {
        projectId,
        OR: [{ name: dto.name }, { promotionOrder: dto.promotionOrder }],
      },
    });
    ...
    if (existing) {
      ...
      throw new ConflictException(
        `Promotion order ${dto.promotionOrder} is already used by "${existing.name}"`,
      );
    }
