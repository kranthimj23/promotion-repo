...
export class ProjectService {
  ...
  async setPromotionRepo(projectId: string, dto: CreatePromotionRepoDto) {
    const existing = await this.prisma.promotionRepo.findUnique({
      where: { projectId },
    });
    if (existing) {
      // Guide says throw, but we could also update. I'll follow the guide.
      throw new ConflictException(
        'Project already has a promotion repo. Update it instead.',
      );
    }
  ...
  async addSourceRepo(projectId: string, dto: CreateSourceRepoDto) {
    ...
    if (existing) {
      throw new ConflictException(
        'This repository URL is already registered in this project',
      );
    }
