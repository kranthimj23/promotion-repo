      ...
      findUnique: jest.fn().mockImplementation((args: any) => {
        ...
        if (where.name)
          return Promise.resolve(
            Array.from(store.values()).find((x) => x.name === where.name) ||
              null,
          );
        if (where.projectId)
          return Promise.resolve(
            Array.from(store.values()).find(
              (x) => x.projectId === where.projectId,
            ) || null,
          );
      ...
      findUniqueOrThrow: jest
        ...
        .mockImplementation((args: any) =>
          mockModel
            ...
            .then(
              (res: any) =>
                res || Promise.reject(new Error(`${model} not found`)),
            ),
        ),
          ...
          return Object.entries(where).every(([key, val]) => {
            if (key === 'OR' && Array.isArray(val)) {
              return val.some((condition) =>
                Object.entries(condition).every(([ck, cv]) => x[ck] === cv),
              );
            }
            return x[key] === val;
          });
